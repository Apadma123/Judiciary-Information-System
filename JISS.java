...
public class JISS {
	...
	public static void main(String[] args) {
		...
		try{
			...
			JOptionPane.showMessageDialog(null, "Database connection error. Please check the following:\nMySQL must be installed."
					+ "\nUser 'JISS' with no password must be created.\nExiting...");
			return;
		}
	...
	private void initialize() {
		frame = new JFrame("Judiciary Information System");
                ...
                try{
                    frame.setContentPane(new JLabel(new ImageIcon(ImageIO.read(new File("C:\\Users\\iiitbasar\\Documents\\NetBeansProjects\\JavaApplication1\\src\\1.jpg")))));
                }
                catch(IOException e){
                    e.printStackTrace();
                }
               /* ImageIcon i=new ImageIcon("j.png");*/
		...
		btnCreateRegistrar.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				Pattern usrNamePtrn = Pattern.compile("^[a-zA-Z0-9_-]{6,14}$");
                                Matcher mtch = usrNamePtrn.matcher(txtRegUsername.getText());
                            if(!mtch.matches()){
				...
				if(String.valueOf(pwdReg_1.getPassword()).isEmpty() || txtRegUsername.getText().isEmpty()){
					...
					return;
                                ...
                                String j=String.valueOf(pwdReg_1.getPassword());
                                System.out.print(j);
                                String upperCaseChars = "(.*[A-Z].*)"; String lowerCaseChars = "(.*[a-z].*)"; String numbers = "(.*[0-9].*)"; String specialChars = "(.*[,~,!,@,#,$,%,^,&,*,(,),-,_,=,+,[,{,],},|,;,:,<,>,/,?].*$)";
				if((j.length()>15 || j.length()<8) || (!j.matches(upperCaseChars)) ||(!j.matches(lowerCaseChars)) || (!j.matches(numbers )) || (!j.matches(specialChars ))){
                                    JOptionPane.showMessageDialog(frame, "Password should contain atleast one special character\nPassword should contain atleast one number\nPassword should contain atleast one lower case alphabet\nPassword should contain atleast one upper case alphabet\nPassword should be less than 15 and more than 8 characters in length.");
                                    return;
                                }
		...
		btnLogin.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
                            Pattern usrNamePtrn = Pattern.compile("^[a-zA-Z0-9_-]{6,14}$");
                                Matcher mtch = usrNamePtrn.matcher(txtUsername.getText());
                            if(!mtch.matches()){
                                       JOptionPane.showMessageDialog(frame, "Please enter valid username"); 
                                       return;
                                  }

                            String j=String.valueOf(txtPassword.getPassword());
                                System.out.print(j);
                                String upperCaseChars = "(.*[A-Z].*)"; String lowerCaseChars = "(.*[a-z].*)"; String numbers = "(.*[0-9].*)"; String specialChars = "(.*[,~,!,@,#,$,%,^,&,*,(,),-,_,=,+,[,{,],},|,;,:,<,>,/,?].*$)";
				if((j.length()>15 || j.length()<8) || (!j.matches(upperCaseChars)) ||(!j.matches(lowerCaseChars)) || (!j.matches(numbers )) || (!j.matches(specialChars ))){
                                    JOptionPane.showMessageDialog(frame, "Password should contain atleast one special character\nPassword should contain atleast one number\nPassword should contain atleast one lower case alphabet\nPassword should contain atleast one upper case alphabet\nPassword should be less than 15 and more than 8 characters in length.");
