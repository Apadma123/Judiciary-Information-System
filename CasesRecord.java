	...
	public void initPanel(User ac){
		...
		btnGo_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				ResultSet rs = JISS.db.getrs("select CIN,defName,Type from cases where dateHearing=\"" + txtDateHearing.getText() + "\"");
				try{
					while(rs.next()){
						for(int j=0; j<3; j++){
							rows[i][j] = rs.getString(j+1);
	...
	private void createCase(){
		...
		try{
			...
			dateArrest = JISS.getDate(txtDateArrest.getText());
                         Pattern usrNamePtrn =  Pattern.compile(new String ("^[a-zA-Z\\s]*$"));
                         Pattern address= Pattern.compile(new String("^\\d+\\s[A-z]+\\s[A-z]+"));
                         Matcher mtch = usrNamePtrn.matcher(txtDefName.getText());
				...
				return;
                            ...
                            if(!mtch.matches()){
                                       JOptionPane.showMessageDialog(panel, "Please enter valid defendant name"); 
                                       return;
                                  }
                            Matcher mt1 = address.matcher(txtDefAddr.getText());
                            Matcher mt2 = usrNamePtrn.matcher(txtTypeCrime.getText());
                            Matcher mt3 = usrNamePtrn.matcher(txtLocation.getText());
                             Matcher mt4 = usrNamePtrn.matcher(txtArrestingOffcr.getText());
                            
                            
                            
                            if(!mt1.matches()){
                                       JOptionPane.showMessageDialog(panel, "Please enter valid address"); 
                                       return;
                                  }
                            if(!mt2.matches()){
                                       JOptionPane.showMessageDialog(panel, "Please enter valid type of crime"); 
                                       return;
                                  }
                            if(!mt3.matches()){
                                       JOptionPane.showMessageDialog(panel, "Please enter valid location"); 
                                       return;
                                  }
                             if(!mt4.matches()){
                                       JOptionPane.showMessageDialog(panel, "Please enter valid addressingofficer"); 
                                       return;
                                  }
                         Date today = new Date();
                         if(dateCrime.compareTo(today)>0){
                             JOptionPane.showMessageDialog(panel, "Date of Crime cannot be earlier than todays date!");
				return;
                         }
